...
describe('normalizeStatsPath', () => {
  ...
  it('spells a story file exactly as Storybook reports its importPath', () => {
    // The Index writes these keys straight into `onlyStoryFiles`, where they are matched against
    // story filenames, so they have to match byte-for-byte.
    expect(normalizeStatsPath('./src/lib/Badge/Badge.stories.tsx', projectRoot)).toBe(
  ...
  it('normalizes a hoisted node_modules path the same way from either spelling', () => {
    // Vite writes `./../../node_modules/...`; rspack writes the absolute path. Both are one file.
    expect(
