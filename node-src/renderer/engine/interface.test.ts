...
import { runTask, TaskConfig, TaskRenderer } from './interface';
...
describe('runTask', () => {
  ...
  it('marks the context skipped and still succeeds on a skip result', async () => {
    ...
    await runTask(
      ctx,
      { name: 'auth', transitions, extractInput: () => ({}), run: async () => ({ kind: 'skip' }) },
      renderer
    );
  ...
  it('falls back to a generic failure headline when no failure transition is provided', async () => {
    ...
    await expect(
      runTask(ctx, { name: 'auth', transitions, extractInput: () => ({}), run }, renderer)
