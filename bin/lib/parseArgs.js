...
export default function parseArgs(argv) {
  const { input, flags, help, pkg } = meow(
    ...
    Chromatic options
      ...
      --externals <filepath>  When any of these files have changed since the baseline build, always run a full build. Used with --only-changed. Globs are supported via picomatch. This flag can be specified multiple times.
